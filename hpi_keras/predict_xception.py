...
def show_samples(test_X, test_Y, pred_Y):
    ...
    for (idx, c_ax) in zip(sickest_idx, m_axs.flatten()):
        ...
        stat_str = [n_class[:6] for n_class, n_score in zip(cfg.label_names, test_Y[idx]) if n_score > 0.5]
        pred_str = ['%s:%2.0f%%' % (n_class[:4], p_score * 100) for n_class, n_score, p_score in
                    zip(cfg.label_names, test_Y[idx], pred_Y[idx])
...
def load_val_gennerator():
    ...
    train_df, val_df = split_train_val(train_val_df, ratio = 0.25, seed=42)
    ...
    for batch_id in range(len(gen)):
        ...
        for i, pred in enumerate(batch_pred):
            ...
            if return_label:
                ...
                print('batch' + str(batch_id + 1) + '_' + str(i), ' prediction: ', np.where(pred_cache > 0)[0], '  gt: ', np.where(batch_y_cache > 0)[0])
                print('predicting batch ', batch_id + 1, ', total', len(gen), '---- accuracy score: ', score)
            else:
                print('batch' + str(batch_id + 1) + '_' + str(i), ' prediction: ', np.where(pred_cache > 0)[0])
...
def eval_roc(task_name, pred_Y, test_Y, all_labels):
    # ROC
    fig, c_ax = plt.subplots(1, 1, figsize=(9, 9))
    for idx, name in all_labels.items():
        fpr, tpr, thresholds = roc_curve(test_Y[:, idx].astype(int), pred_Y[:, idx])
        print(name, ': threshold is ', thresholds)
    ...
    for idx, name in all_labels.items():
        ...
        print(
            '%s: 预测阳性比例: %2.2f%%, 实际阳性比例: %2.2f%%, acc: %2.2f' % (name, pred_pos[idx] * 100, test_pos[idx] * 100, acc))
...
def eval_f1score(task_name, pred_Y, test_Y, all_labels):
    ...
    for idx, name in all_labels.items():
        score_idx = f1_score(test_Y[:, idx], (pred_Y > threshold)[:, idx], average='macro')
...
def eval(task_name, epoch):
    ...
    eval_roc(task_name, pred_Y, test_Y, cfg.label_names)
    threshold = eval_f1score(task_name, pred_Y, test_Y, cfg.label_names)

    return threshold

def submission(task_name, epoch, submission_file, threshold):
...
def task1():
    task_name = 'xception3'
    epoch = 37
    submission_file = 'xception_gap3.csv'
    threshold = eval(task_name, epoch)
    print(threshold)
    submission(task_name, epoch, submission_file, threshold)


def task2():
    task_name = 'xception2'
    epoch = 37
    submission_file = 'xception_gap2.csv'
    threshold = eval(task_name, epoch)
    print(threshold)
    submission(task_name, epoch, submission_file, threshold)
...
def task3():
    task_name = 'xception1'
    epoch = 41
    submission_file = 'xception_gap1.csv'
    threshold = eval(task_name, epoch)
    print(threshold)
    submission(task_name, epoch, submission_file, threshold)
